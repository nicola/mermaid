...
export const drawActors = function (diagram, actors, actorKeys, verticalPos) {
  ...
  for (let i = 0; i < actorKeys.length; i++) {
    ...
    svgDraw.drawActor(diagram, actors[key].x, verticalPos, actors[key].description, conf, actor, maxLevel)
    bounds.insert(actors[key].x, verticalPos - actor.level * 20, actors[key].x + conf.width, conf.height)
  }

  // Add a margin between the actor boxes and the first arrow
  bounds.bumpVerticalPos(conf.height + maxLevel * 10)
}
